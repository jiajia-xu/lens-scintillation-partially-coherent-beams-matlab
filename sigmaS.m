...
%%
omiga_xx=((1/(4*sigma_S(i)^2))+(1/(delta_xx^2)))^(-0.5);
omiga_yy=((1/(4*sigma_S(i)^2))+(1/(delta_yy^2)))^(-0.5);
omiga_xy=((1/(4.*sigma_S(i).^2))+(1./(delta_xy.^2))).^(-0.5);
...
%%
%hold on
legend('\sigma_{S}=0.01','','\sigma_{S}=0.1','','\sigma_{S}=1','','\sigma_{S}=10','','\sigma_{S}=100','')
