...
%%
Sintillation_in=((Ax.^2).^2+2.*(Ax.*Ay(i).*Bxy).^2+(Ay(i).^2).^2)./((Ax.^2+Ay(i)^2).^2);
%%
omiga_xx=((1/(4*sigma_S^2))+(1/(delta_xx^2)))^(-0.5);
omiga_yy=((1/(4*sigma_S^2))+(1/(delta_yy^2)))^(-0.5);
omiga_xy=((1/(4.*sigma_S.^2))+(1./(delta_xy.^2))).^(-0.5);
Sintillation_focus=((Ax.^2.*omiga_xx).^2+2.*(Ax.*Ay(i).*Bxy.*omiga_xy).^2+(Ay(i).^2.*omiga_yy).^2)./((Ax.^2.*omiga_xx+Ay(i)^2.*omiga_yy).^2);
...
%%
%hold on
legend('Ax/Ay=4','','Ax/Ay=2','','Ax/Ay=1','','Ax/Ay=2/5','','Ax/Ay=1/5','')
