clear
clc
%改变Bxy=1 for a fully polarized source
%%
sigma_S=0.01; %the effective beamwidth
delta_xx=0.03;
%delta_yy=0.045;
delta_yy=delta_xx/1.2;
Ax=1;
Ay=1;
Bxy=1; 
%%
%for i=1:5
%%   ??????????????????
T1=(delta_xx^2+delta_yy^2)/2;
min=((delta_xx^2+delta_yy^2)/2)^0.5;
T2=(delta_xx*delta_yy)/Bxy;
max=((delta_xx*delta_yy)/Bxy)^0.5;
%mmax=((delta_xx*delta_yy)/Bxy)^0.5+0.0001;
N=300;
delta_xy=linspace(min,max,N);                                                           
Sintillation_focus=zeros(N);
%delta_xy=min:0.0001:max;
%%
Sintillation_in=((Ax.^2).^2+2.*(Ax.*Ay.*Bxy).^2+(Ay.^2).^2)./((Ax.^2+Ay^2).^2);
%%
omiga_xx=((1/(4*sigma_S^2))+(1/(delta_xx^2)))^(-0.5);
omiga_yy=((1/(4*sigma_S^2))+(1/(delta_yy^2)))^(-0.5);
omiga_xy=((1./(4.*sigma_S.^2))+(1./(delta_xy.^2))).^(-0.5);
Sintillation_focus=((Ax.*omiga_xx).^4+2.*(Ax.^2.*Ay.^2.*Bxy.^2.*omiga_xy.^4)+(Ay.*omiga_yy).^4)./((Ax.^2.*omiga_xx.^2+Ay.^2.*omiga_yy.^2).^2);
P=( 1 - (4.*(Ax.^2.*Ay.^2.*omiga_xx.^2.*omiga_yy.^2  - Ax.^2 .*Ay.^2 .*Bxy.^2 .*omiga_xy.^4  )./(Ax^2.*omiga_xx.^2 + Ay^2.*omiga_yy.^2 ) )   ).^0.5;
%Sintillation_focus=(1/2).*(1+P.^2);
%%
plot(delta_xy,P,'Linewidth',2.5)
hold on
%%
% Ox=[min, max];
% Oy=[Sintillation_in, Sintillation_in];
% plot(Ox,Oy,'Linewidth',2.5,'LineStyle','--');

%Oz=[1,2];
%p1.LineWidth = 1;
%%
% ax = gca();
% ax.YRuler.Exponent = -4; 
% ax.YRuler.TickLabelFormat = '%.3f';
% %
% ax = gca();
% ax.XRuler.Exponent = -2; 
% ax.XRuler.TickLabelFormat = '%.3f';
%hold on
%%
xlabel('\delta_{xy}','Fontsize',20,'Fontname','Times');
ylabel('\sigma_{\itF}^2','Fontsize',20,'Fontname','Times');
set(gca,'fontname','Times New Roman','FontSize',16);
set(gca,'fontname','Times New Roman','FontSize',16);
title('For a fully polarized source B_{\itxy}=1','fontname','Times New Roman','FontSize',20)
%%
%set(gca,'XTick',[min,0.033,0.036,0.039,0.042,0.045,mmax],'fontname','Times New Roman','FontSize',20);
%set(gca,'YTick',[0.5,0.6,0.7,0.8,0.9,1],'fontname','Times New Roman','FontSize',20);
%xlim([min,mmax]),
%ylim([0.5,1])

%set(gca,'xTickLabel',num2str(get(gca,'XTick')'))
%set(gca,'yTickLabel',num2str(get(gca,'YTick')'))
%xtickformat('%.4f')

%ylim([0,1]);
%set(gca,'Linewidth',3,'Fontname','Times','Fontsize',17,'Fontweight','Bold','Ytick',[0,0.5,1],'Color','blue',);

%%
%hold on
%legend('|B_{xy}|=0.4','','|B_{xy}|=0.5','','|B_{xy}|=0.6','','|B_{xy}|=0.7','','|B_{xy}|=0.8','')
