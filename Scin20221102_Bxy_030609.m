clear
clc
%改变Bxy=0.3 0.5 0.8 for a partially polarized source
% Ax= Ay
...
mmax=((delta_xx*delta_yy)/Bxy(1))^0.5+0.01;
N=300;
...
%%
%Find the delta_xy max
%Find the final sintillation index
delta_max_xy=((delta_xx*delta_yy)/Bxy(3))^0.5;
omiga_xy_Bxy=((1/(4.*sigma_S.^2))+(1./(delta_max_xy.^2))).^(-0.5);
Sintillation_Bxy_focus=((Ax.*omiga_xx).^4+2.*(Ax.^2.*Ay.^2.*Bxy(3).^2.*omiga_xy_Bxy.^4)+(Ay.*omiga_yy).^4)./((Ax.^2.*omiga_xx.^2+Ay.^2.*omiga_yy.^2).^2);
%%
...
xlabel('\delta_{\itxy}  [mm]','Fontsize',26,'Fontname','Times');
ylabel('\sigma^2_{\itF}','Fontsize',26,'Fontname','Times');
...
%title('\sigma^2 for a partially polarized source with A_{\itx}=A_{\ity}','fontname','Times New Roman','FontSize',20)
%title('For a partially polarized source with A_{\itx}=A_{\ity}','fontname','Times New Roman','FontSize',20)

%%
%hold on
%legend('|{\itB}_{\itxy}| = 0.3','','|{\itB}_{\itxy}| = 0.5','','|{\itB}_{\itxy}| = 0.8','')
